(async () => {
 ...
 try {
   ...
   ['fbclid','ttclid','gclid','utm_source','utm_medium','utm_campaign','utm_content','utm_term']
     .forEach(function(k) { if(_sp.get(k)) _octoTracking[k] = _sp.get(k); });
   document.cookie.split(';').forEach(function(c) {
     var p = c.trim().split('=');
     if(p[0]==='_fbp'||p[0]==='_fbc') _octoTracking[p[0]] = p[1];
   });
   _octoTracking.landing_page = window.location.pathname;
   _octoTracking.referrer = document.referrer || '';
   var _prev = {};
   try { _prev = JSON.parse(sessionStorage.getItem('_octo_tracking')||'{}'); } catch(e){}
   var _merged = Object.assign({}, _prev, _octoTracking);
   sessionStorage.setItem('_octo_tracking', JSON.stringify(_merged));
 } catch(e) {}
...
<span style="font-size:${fs(8)}px;color:${annTextColor};margin-top:2px;text-transform:uppercase;">${seg.label}</span>
 ...
 // Hide any already-existing native drawers using targeted selectors
 const drawerSelectors = [
   'cart-drawer', 'cart-notification', 'cart-notification-drawer',
   'side-cart', 'ajax-cart', 'mini-cart',
   '[id*="cart-drawer" i]', '[id*="CartDrawer" i]', '[id*="cart-notification" i]',
   '[class*="cart-drawer" i]', '[class*="mini-cart" i]', '[class*="minicart" i]',
   '[class*="cart-flyout" i]', '[class*="cart-sidebar" i]',
   '[data-section-type*="cart"]'
 ].join(',');
 try {
   document.querySelectorAll(drawerSelectors).forEach(el => {
...
<div id="cf-continue-wrap" style="text-align:center;margin-top:8px;"></div>
 ...
 var _ckBtn = document.getElementById('cf-checkout');
 if (_ckBtn) { _ckBtn.disabled = false; }
 }
 function closeCart() {
...
<div style="color:${savingsColor};font-weight:600;">
 ...
 // === TRACKING INJECTION ===
 var _td = {};
 try { _td = JSON.parse(sessionStorage.getItem('_octo_tracking')||'{}'); } catch(e){}
 var _allT = Object.assign({}, _td);
 try { new URLSearchParams(window.location.search).forEach(function(v,k){ _allT[k]=v; }); } catch(e){}
 ['fbclid','ttclid','gclid','utm_source','utm_medium','utm_campaign','utm_content','utm_term',
  '_fbp','_fbc','landing_page','referrer'].forEach(function(k) {
   if(_allT[k]) checkoutUrl += (checkoutUrl.includes('?')?'&':'?') + 'attributes%5B'+k+'%5D='+encodeURIComponent(_allT[k]);
 });
 checkoutUrl += (checkoutUrl.includes('?')?'&':'?') + 'attributes%5Bsource%5D=octoroute';
 if(_allT.fbclid) checkoutUrl += '&fbclid='+encodeURIComponent(_allT.fbclid);
 if(_allT.ttclid) checkoutUrl += '&ttclid='+encodeURIComponent(_allT.ttclid);
 return checkoutUrl;
 }
 ...
 if (cartEl && !cartEl.closest('#cf-drawer') && !cartEl.closest('#cf-overlay')) {
   // EXCLUSIONS: skip add-to-cart buttons and product context
   const elText = (cartEl.innerText || cartEl.textContent || '').trim().toLowerCase();
   const isAddToCart = /add|buy|comprar|adicionar|kaufen|ajouter/.test(elText);
   const isInProductCtx = cartEl.closest('[data-section-type="product"], .product-form, product-info, .product__info, [action*="/cart/add"], form[action*="/cart"]');
   const isSubmit = cartEl.getAttribute('type') === 'submit' || cartEl.getAttribute('name') === 'add';
   if (isAddToCart || isInProductCtx || isSubmit) {
     // Do NOT intercept — let the theme handle this click
   } else {
     ...
     const isInHeader = cartEl.closest('header, nav, .header, .site-header, .shopify-section-header, #shopify-section-header, [data-section-type="header"]');
     if (isInteractive && isSmall && isInHeader) {
       e.preventDefault();
       if(window._cfConfig && window._lastCart) renderCart(window._lastCart, window._cfConfig);
       openCart();
       const cart = await fetchShopifyCart();
 ...
 const [config] = await Promise.all([getConfig(initialSkus), getVitrineSkuMap()]);
 ...
 console.log('[CartFlow] ✓ Loaded (v7)');
