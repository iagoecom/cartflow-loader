...
[data-cart-drawer],[data-mini-cart],.side-cart,.slide-cart,.cart-sidebar
  ...
  window.cfUpdateUpsellVariant = function(selectEl) {
    ...
    console.log('[CF] cfAddUpsell', productId, 'found:', !!_allUpsells.find(p => p.id === productId), 'pending:', _upsellPending);
    const product = _allUpsells.find(p => p.id === productId);
    if (!product) return;
...
if (triggers.some(sel => { try { return t.matches?.(sel)||t.closest?.(sel); } catch(e) { return false; } })) {
  ...
  try {
    ...
    console.log('[CartFlow] ✓ Loaded v13.0 (zero-latency-upsells)');
